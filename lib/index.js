...
const register = async (server, pluginOptions) => {
  ...
  const connect = async connectionOptions => {
    ...
    if (typeof connectionOptions.decorate === 'string') {
      const decoration = Object.assign({ odbpool: odbpool }, expose)
  ...
  let odbpools = []
  try {
    odbpools = await Promise.all(finalOpts.map(connect))
  } catch (err) {
    server.log(['oracledb', 'error'], err)
  }

  expose.odbpool = finalOpts.length === 1 ? odbpools[0] : odbpools
  ...
  server.events.on('stop', () => {
    odbpools.forEach(async pool => {
      try {
        console.log(`closing oracle pooling`)
        await pool.close()
        server.log(['oracledb', 'info'], `closing oracle pooling`)
      } catch (err) {
        server.log(['oracledb', 'error'], err)
      }
    })
